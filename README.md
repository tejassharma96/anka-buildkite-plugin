...
| `anka-interpreter` | Shell (or other interpreter) used to execute `BUILDKITE_COMMAND` inside the VM. The command string is passed as the final argument, so include `-c` (or equivalent) if using a shell. Defaults to `bash -l -c`. | `zsh -l -c` |
